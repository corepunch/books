> Historical reference. The Lua/XML host, export steps and paths described
> here are retired. See ../PROCESS.md and ../RENDERING.md for current work.
...
Establishing views should normally look diagonally into a room junction rather
than stand upright and square to one wall. Show two receding architectural
planes, stage useful objects or exits along both axes, and use a slightly high
or low viewpoint when it strengthens depth. A foreground edge may frame the
view, but it must not hide an affordance. Reserve frontal coverage for a close-up
whose object is clearer that way.

Create text space with composition and selective illumination. Let one wall or
floor plane fall into a narrow, low-contrast value range while a motivated key
separates the focal objects against it. Do not fill an entire empty wall merely
to make room for prose, and do not allow hard shadows to become stronger shapes
than the subjects casting them.
...
shavings, outlines, and object tangencies away from letterforms. Put the richest
accents outside the reading fields.
...
All generated production assets live under the Book root's `illustrations/`
directory. They must not remain only in the local generation cache or in the
zilscript text-adventure source tree.
