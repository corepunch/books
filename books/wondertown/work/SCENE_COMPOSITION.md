...
### Object bounds and contact checks
...
Render and inspect the named object groups from the engine root:

```sh
make render BOOK=wondertown SCENE=workshop-new
make layout BOOK=wondertown SCENE=workshop-new
make check
```

`make check` validates host projection and interaction, not geometry collisions.
Use Scener's authoring views and transformed world bounds to inspect clearances
to wall faces, floor and ceiling, and distances to other assemblies. Include
