...
# Derive shot names from the VM's declared IDs and owning rooms, not a manual map.
engine = BOOK.parents[1]
catalog = json.loads(subprocess.check_output([
    str(engine / 'build/book'), '--root', str(engine), '--book', BOOK.name, '--catalog'
], text=True))
...
save(ROOMS/'workshop-new.blks',s,'Fresh workshop. X east, Y north, Z up; cm. Authoring source: books/wondertown/work/tools/build_workshop_new.py. Initial story state.')
