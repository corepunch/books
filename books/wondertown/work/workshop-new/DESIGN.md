...
Review `books/wondertown/rooms/workshop-floor-look.jpg` and `contact.jpg` for
...
launches the single-file C engine with existing JPEGs. Scener generates the
images offline; `main.c` reads `.blks` cameras and named anchors solely for
hotspot projection, preserving the source Z-up coordinate system. No camera
