...
local source_path = require("Book.Scripts.WorkshopInteractions").scene_path
...
function Session.new(options)
    ...
    local file = assert(io.open(options.bootstrap_path or "zilscript/bootstrap.lua", "r"))
    ...
    assert(runtime.load_modules(env, {options.story_module or "Book.Scripts.WondertownPrototype"}, {silent = true}),
    ...
    return setmetatable({
        env = env, game = game, objects = objects, scenes = scenes, render_dir = options.render_dir,
...
function Session:view()
    ...
    if self.focus then
        ...
        local cam, img = coc(self, room, self.focus)
        local choices = {}
...
function Session:tap(name)
    ...
    local cam, img = coc(self, room, canonical(name))
